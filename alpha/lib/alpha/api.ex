defmodule Alpha.Api do
  @doc """
  Starts worker and registers name in the cluster, then joins the process
  to the `:foo` group
  """
  def start_worker(name) do
    {:ok, pid} = Swarm.register_name(name, Alpha.UserSupervisor, :register, [name])

    Swarm.join(:foo, pid)
  end

  def xavi_start(), do: start_worker("xavi")
  def miquel_start(), do: start_worker(:miquel)
  def gorka_start(), do: start_worker(:gorka)
